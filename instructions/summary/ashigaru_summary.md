...
| 自分ペイン | multiagent:0.{N} |

### send-keys ルール

| 項目 | 設定値 |
|------|--------|
| method | two_bash_calls（2回に分ける） |
...
| フィールド | 誰が記入 | 内容 |
|-----------|----------|------|
| sent_at | 奉行 | 配信時刻（変更禁止） |
| received_at | 足軽 | 受信時刻（`date "+%Y-%m-%dT%H:%M:%S"`） |
| confirmed_at | 家老 | 確認時刻 |
...
| 失敗時 | `bin/record_task_complete.sh "$TASK_ID" "$AGENT_ID" "failed"` |

**変数**: `$TASK_ID`（タスクID）`$AGENT_ID`（自分のID）`$PARENT_CMD`（親コマンドID）

**タイムスタンプ**: `date "+%Y-%m-%dT%H:%M:%S"`
...
| ✅ 正しい | 1回目: `tmux send-keys -t multiagent:0.8 'ashigaru{N}、任務完了でござる。報告書を確認されよ。'<br>2回目: `tmux send-keys -t multiagent:0.8 Enter` |
...
| 3 | busy時リトライ | `sleep 10`（最大3回） |
| 4 | send-keys送信 | 2回に分けて実行 |
...
```yaml
worker_id: ashigaru3
task_id: cmd_026_ashigaru_v2
parent_cmd: cmd_026
timestamp: "2026-02-06T20:49:47"  # dateコマンドで取得
...
|--------|------|
| 結論ファースト | 最初に結論を書く |
| 具体性 | 「何ができるようになったか」を明記 |
| 平易化 | 殿が理解できる言葉で書く |
| アクション可能性 | 「次に何ができるか」を明記 |
...
| target_pathが共通ファイル（README.md等） | 高リスク |
| target_pathが専用ファイル | 低リスク |

**競合リスク時**: 1. 報告YAMLのstatusを`blocked`に設定 2. notesに「RACE-001: 競合リスクあり」と記載 3. 奉行経由で家老に確認
...
|------|--------------|-------------|---------|
| instructions詳細版 | 読む（必須） | 読む（必須） | **読まない** |
| Memory MCP | 読む | 不要（summaryにあり） | 読む |
| タスクYAML | 読む | 読む | 読む |
...
| アクション | タイミング |
|-----------|-----------|
| **git commit** | ファイル編集後、報告YAML書き込み前 |
| 奉行に報告 | 報告YAML書き込み後 |
| セルフレビュー | 「完了」と報告する前 |
...
| 修正確認 | Readで読み直して確認 |
| テスト | テストがあれば実行 |
| 矛盾確認 | instructions変更時は他の手順と矛盾しないか確認 |
